...
return({{extName}});
  ...
  onMount(() => {
    const unsubConfig = config.subscribe(value => {
      ...
      if(typeof localConfig.localFS !== 'undefined') {
          extensionList = localConfig.localFS.getDirList(localConfig.localFS.appendPath(localConfig.configDir,'extensions'));
      }
    });
  ...
  function editExtension(ext) {
    ...
    //
    var extDir = localConfig.localFS.appendPath(localConfig.configDir,'extensions');
    extDir = localConfig.localFS.appendPath(extDir, ext.name);
    var pkgConfig = JSON.parse(localConfig.localFS.readFile(localConfig.localFS.appendPath(extDir, 'package.json')));
    if(typeof pkgConfig.mfmextension !== 'undefined') {
      ...
      if(localConfig.localFS.fileExists(localConfig.userEditor)) {
        //
        // There is an editor defined by the user. Use it.
        //
        var file = localConfig.localFS.appendPath(extDir, extFile);
        var editor = localConfig.localFS.readFile(localConfig.userEditor).toString().trim();
        if(editor.endsWith('.app')) {
          localConfig.localFS.openFileWithProgram(editor, file);
        } else {
          //
          ...
          if(editor === 'emacs') {
            ...
            // Open emacs.
            //
            localConfig.localFS.runCommandLine('emacsclient -n -q "' + file + '"');
          } else {
            //
            // Open in a terminal program.
            //
            localConfig.localFS.openInTerminal(editor, file);
          }
        }
      } else {
        //
        ...
        //
        localConfig.localFS.runCommandLine('open "' + file + '"');
      }
  ...
  function createExtension() {
    var extDir = localConfig.localFS.appendPath(localConfig.configDir,'extensions');
    extDir = localConfig.localFS.appendPath(extDir, extensionName);
    if(!localConfig.localFS.dirExists(extDir)) {
      localConfig.localFS.createDir(extDir);
      localConfig.localFS.runCommandLine('cd "' + extDir + '"; npm init -y;', (err, stdout) => {
        if(err) {
          // 
  ...
  function editPackage() {
    ...
    // 
    var fileName = localConfig.localFS.appendPath(extDir, 'package.json');
    var pkgConfig = JSON.parse(localConfig.localFS.readFile(fileName));
    ...
    localConfig.localFS.writeFile(localConfig.localFS.appendPath(extDir, 'package.json'), JSON.stringify(pkgConfig));
    
    // 
    // Create the actual extension file.
    // 
    var extFile = localConfig.localFS.appendPath(extDir,extensionName + '.js');
    var newExt = extTemplate.replaceAll('{{extName}}', extensionName);
    localConfig.localFS.writeFile(extFile, newExt);
  ...
  function deleteExtension(ext) {
    var extDir = localConfig.localFS.appendPath(localConfig.configDir,'extensions');
    localConfig.localFS.deleteEntries({
      dir: extDir,
      name: ext.name,
      fileSystem: localConfig.localFS
    }, (err,stdout) => {
